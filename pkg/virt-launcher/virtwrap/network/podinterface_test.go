...
var _ = Describe("Pod Network", func() {
	...
	Context("Masquerade startDHCP", func() {
		...
		It("should succeed when DHCP server started", func() {
			...
			mockNetwork.EXPECT().CreateRADaemon(expectedSocketPath, api.DefaultBridgeName, api.DefaultVMIpv6CIDR, gomock.Any(), maxRetryNumber).Return(nil)
			mockNetwork.EXPECT().LinkByName(api.DefaultBridgeName).Return(bridgeTest, nil)
		...
		It("should fail when RA daemon failed", func() {
			...
			mockNetwork.EXPECT().CreateRADaemon(expectedSocketPath, api.DefaultBridgeName, api.DefaultVMIpv6CIDR, gomock.Any(), maxRetryNumber).Return(err)
			mockNetwork.EXPECT().LinkByName(api.DefaultBridgeName).Return(nil, err)
